-- Devolucao parcial: quantity em returns, sale_item_id para teto por linha, estoque += quantity.
...
create index if not exists returns_tenant_sale_item_idx
  on public.returns (tenant_id, sale_item_id)
...
begin
  ...
  if v_qty <= 0 then
    raise exception 'Quantidade invalida na devolucao (estoque).';
  end if;
...
-- register_sale_flow: p_return_quantity; amount = credito total (unit x qtd ou valor custom informado).

-- Formulário baseado nos campos do contato + "Outras informações" + "Canal de origem".
--  - other_info: respostas do formulário que não correspondem a campo do cadastro.
--  - canal_origem (customers.source_columns): canal do PRIMEIRO contato (first-touch),
--    'WhatsApp' (lead vindo de chat) ou 'Formulário' (submit). Nunca sobrescrito.

-- Campo "Outras informações" na negociação
alter table public.crm_negotiations
  add column if not exists other_info jsonb not null default '{}'::jsonb;

-- =====================================================================
-- Canal de origem first-touch p/ leads vindos do WhatsApp
-- (negociação criada com source_chat_id => veio de um chat)
-- =====================================================================
create or replace function public.set_customer_channel_on_negotiation()
returns trigger
language plpgsql
security definer
...
as $$
begin
  if NEW.source_chat_id is not null and NEW.customer_id is not null then
    update public.customers c
    set source_columns = jsonb_set(coalesce(c.source_columns, '{}'::jsonb), '{canal_origem}', '"WhatsApp"', true)
    where c.id = NEW.customer_id
      and coalesce(c.source_columns->>'canal_origem', '') = '';
  end if;
  return NEW;
end;
$$;

drop trigger if exists crm_negotiations_set_channel on public.crm_negotiations;
create trigger crm_negotiations_set_channel
after insert on public.crm_negotiations
for each row execute function public.set_customer_channel_on_negotiation();

-- =====================================================================
-- submit_marketing_form: roteia respostas
--   default (nome/email/telefone) -> customer
--   custom (mapping.fieldId)      -> customer_custom_field_values
--   extra                         -> crm_negotiations.other_info
-- + canal_origem='Formulário' (first-touch)
-- =====================================================================
...
begin
  ...
  if v_customer_id is null and v_email is not null then
    select c.id into v_customer_id
    from public.customers c
    where c.tenant_id = v_tenant and lower(nullif(c.email, '')) = v_email
    order by c.updated_at desc
    limit 1;
  end if;
  ...
  if v_neg_id is null then
    insert into public.crm_negotiations (
      tenant_id, title, funnel_id, stage_id, status, customer_id, last_interaction_at, last_contact_at
    ) values (
      v_tenant, v_title, v_funnel, v_stage, 'em_andamento', v_customer_id, timezone('utc', now()), timezone('utc', now())
    )
    returning id into v_neg_id;
  else
    update public.crm_negotiations n
    set last_interaction_at = timezone('utc', now()), last_contact_at = timezone('utc', now())
    where n.id = v_neg_id;
  end if;
  ...
  -- Roteia respostas por mapeamento do campo
  if jsonb_typeof(v_form.fields) = 'array' then
    for v_field in select * from jsonb_array_elements(v_form.fields) loop
      ...
      -- campos do cadastro padrão: já gravados no customer
      if v_mkind = 'default'
        ...
        select kind into v_ckind from public.customer_custom_fields where id = v_field_uuid and tenant_id = v_tenant;
        ...
        values (
          v_customer_id, v_field_uuid,
          case when v_storage = 'text' then
            case when v_ckind = 'booleano'
              then (case when lower(v_fvalue) in ('1', 'sim', 'true', 'on', 'yes') then '1' else '0' end)
              else v_fvalue end
          else null end,
          case when v_storage = 'numeric'
            then nullif(regexp_replace(replace(replace(v_fvalue, '.', ''), ',', '.'), '[^0-9.\-]', '', 'g'), '')::numeric
          else null end,
  ...
  update public.marketing_forms set total_submissions = total_submissions + 1 where id = p_form_id;

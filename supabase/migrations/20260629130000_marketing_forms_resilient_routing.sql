-- Torna o roteamento de campos do submit_marketing_form resiliente.
--
-- Regressão: a migration 20260628320000 ("Restaura o roteamento") reintroduziu o loop
-- que grava campos custom em customer_custom_field_values com casts ::date/::numeric e
-- atualiza crm_negotiations.other_info. Esses casts/inserts não eram protegidos: se um
-- valor enviado não casa com o tipo do custom field alvo (ex.: 'Cardiopatia grave'::date),
-- a exceção aborta a RPC inteira e faz rollback do lead já criado — o público vê
-- "Não foi possível enviar no momento" (500).
--
-- Aqui o processamento de cada campo passa a rodar dentro de um bloco BEGIN/EXCEPTION
-- (subtransação): uma falha num campo é registrada como warning e o campo é ignorado,
-- mas o contato, a negociação e as respostas (já gravados antes do loop) são preservados.
...
begin
  ...
  if jsonb_typeof(v_form.fields) = 'array' then
    for v_field in select * from jsonb_array_elements(v_form.fields) loop
      ...
      -- Isolado por campo: um cast/insert que falhe (ex.: valor incompatível com o tipo
      -- do custom field) não pode abortar a submissão inteira nem perder o lead.
      begin
        if v_mkind = 'default'
